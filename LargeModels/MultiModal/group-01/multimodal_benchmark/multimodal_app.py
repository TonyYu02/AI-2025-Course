...
class BailianAPI:
    ...
    def call_api(self, messages: list, stream: bool = False) -> Tuple[str, float]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:  # 最后一次重试失败
                    return f"API调用失败 (尝试{max_retries}次): {str(e)}", 0.0
                else:
        ...
        try:
            ...
            return f"处理响应时出错: {str(e)}", response_time
...
def process_multimodal_input(text: str, image: Optional[Image.Image], api_key: str, model_name: str) -> str:
    ...
    if image is not None:
        try:
            ...
            print(f"DEBUG: 对话图像已编码，大小: {len(image_base64)} 字符")  # 调试信息
        except Exception as e:
            print(f"DEBUG: 对话图像编码失败: {str(e)}")  # 调试信息
...
def run_benchmark_test(api_key: str, test_prompts: List[str]) -> Dict:
    ...
    for model_name in api_client.available_models.keys():
        ...
        for prompt in test_prompts:
            ...
            model_results["response_lengths"].append(len(response) if isinstance(response, str) else 0)
...
def run_multimodal_benchmark_test(api_key: str, test_items: List[Dict]) -> Dict:
    """运行多模态推理评测，比较不同模型的性能"""
    if not api_key.strip():
        return {"error": "请输入API密钥"}
    ...
    for model_name in api_client.available_models.keys():
        ...
        for item in test_items:
            ...
            # 添加图像内容（如果是多模态任务）
            if item["type"] == "multimodal" and item["image"] is not None:
                try:
                    image_base64 = api_client.encode_pil_image_to_base64(item["image"])
                    content.append({
                        "type": "image_url",
                        "image_url": {
                            "url": f"data:image/png;base64,{image_base64}"
                    ...
                    print(f"DEBUG: 图像已编码，大小: {len(image_base64)} 字符")  # 调试信息
                except Exception as e:
                    print(f"DEBUG: 图像编码失败: {str(e)}")  # 调试信息
                    continue
            ...
            # 调用API
            print(f"DEBUG: 发送消息到API，内容类型: {[c.get('type') for c in content]}")  # 调试信息
            response, response_time = api_client.call_api(messages, stream=False)
...
def create_benchmark_results_table(results: Dict) -> str:
    ...
    for _, data in results.items():
        # 计算输出效率 (字符/秒)
        efficiency = data['stats'].get('avg_len', 0) / data['avg_response_time'] if data['avg_response_time'] > 0 else 0
        rows_html += f"""
        <tr>
...
def create_benchmark_bar_chart(results: Dict):
    ...
    fig = go.Figure()
    fig.add_bar(x=models, y=avg, name="响应时间", marker_color="#6366F1")
    fig.update_layout(
        height=380,
...
def create_interface():
    ...
    with gr.Blocks(title="多模态大模型展示与评测", theme=gr.themes.Soft()) as interface:
        ...
        基于百炼平台API的Qwen2.5-VL系列模型多模态对话与性能评测系统
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        benchmark_tasks = gr.CheckboxGroup(
                            ...
                            value=[],
                            label="选择评测任务",
                            info="选择要评测的任务类型（包含文本、图像任务）"
                        )
                        
                        # 图像上传区域（用于图像类评测任务）
                        benchmark_image = gr.Image(
                            label="评测图像上传（用于图像描述、分析、问答等评测任务）",
                            type="pil",
                            height=200
                        )
                        
                        # 开始评测按钮
                        benchmark_btn = gr.Button("开始评测", variant="secondary", size="lg")
                    ...
                    with gr.Column(scale=2):
                        # 评测结果
                        benchmark_chart = gr.Plot(label="响应时间对比图")
        ...
        **评测功能：**
        ...
        # 评测功能
        def run_benchmark(api_key, selected_tasks, uploaded_image):
            if not selected_tasks:
                return None, "<p style='color: red;'>请至少选择一个评测任务</p>"
            
            # 检查图像任务但未上传图像
            image_tasks = [task for task in selected_tasks if "图像" in task]
            if image_tasks and uploaded_image is None:
                return None, "<p style='color: red;'>选择了图像类评测任务，请先上传图像</p>"
            
            # 构建评测任务
            test_items = []
            for task in selected_tasks:
                if ":" in task:
                    task_name = task.split(":", 1)[0].strip()
                ...
                # 判断是否为图像任务
                if "图像" in task_name and uploaded_image is not None:
                    test_items.append({
                        "type": "multimodal",
                        "task_name": task_name,
                        "prompt": prompt,
            ...
            # 运行评测
            results = run_multimodal_benchmark_test(api_key, test_items)
            ...
            for model_name, data in results.items():
                if model_name != "error":
                    ...
                    for i, result in enumerate(data["responses"]):
                        task_type = "🖼️ 图像任务" if result.get("type") == "multimodal" else "📝 文本任务"
                        detailed_results += f"""
                        <li>
                            <strong>{task_type} {i+1}:</strong> {result['task_name']}<br>
                            <strong>提示:</strong> {result['prompt']}<br>
        ...
        benchmark_btn.click(
            fn=run_benchmark,
            inputs=[benchmark_api_key, benchmark_tasks, benchmark_image],
